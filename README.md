# BASIC UNIX COMMANDS PRACTISE OUTPUT
## GENERAL  COMMANDS OUTPUTS 
### DATE COMMAND OUTPUT
![date command output](mdate.png)   
### DATE COMMAND MANUAL
![date command manual](manualdate.png.png)
### DATE TIME COMMAND OUTPUT
![date time command output](mdate1.png)
### DATE TIME COMMAND MANUAL
![date time command manual](manualdate.png.png)
### DATE YEAR COMMAND OUTPUT
![date year command output](mdate2.png)
### DATE YEAR COMMAND MANUAL
![date year command manual](manualdate.png.png)
### DATE HOUR COMMAND OUTPUT
![date hour command output](mdate3.png)
### DATE HOUR COMMAND MANUAL
![date hour command manual](manualdate.png.png)
### CAL COMMAND OUTPUT
![cal command output](mcal.png)
### CAL COMMAND MANUAL
![cal command manual](manualcal.png.png)
### CAL-YEAR COMMAND OUTPUT
![cal-year command output](mcalyear.png)
### CAL-YEAR COMMAND MANUAL
![cal-year command manual](manualcal.png.png)
### CAL-MONTH COMMAND OUTPUT
![cal-month command output](mcalmonthyear.png)
### CAL-MONTH COMMAND MANUAL
![cal-month command manual](manualcal.png.png)
### WHO COMMAND OUTPUT 
![who command output](mwho.png)
### WHO COMMAND MANUAL
![who command manual](manualwho.png.png)
### WHO AM I COMMAND OUTPUT 
![who am i command output](mwhoami.png)
### WHO AM I COMMAND MANUAL
![who am i command manual](manualwho.png.png)
### TTY COMMAND OUTPUT -TERMINAL NAME-
![tty command output](mtty.png)
### TTY COMMAND MANUAL
![tty command manual](manualtty.png.png)
### UNAME COMMAND OUTPUT
![uname command output](uname.png)
### UNAME COMMAND MANUAL
![uname command manual](manualuname.png.png)
### UNAME-R COMMAND OUTPUT
![uname-r command output](muname-r.png)
### UNAME-R COMMAND MANUAL
![uname-r command manual](manualuname.png.png)
### UNAME-N COMMAND OUTPUT
![uname-n command output](muname-n.png)
### UNAME-N COMMAND MANUAL
![uname-n command manual](manualuname.png.png)
### ECHO COMMAND OUTPUT
![echo command output](mechotxt.png)
### ECHO COMMAND MANUAL
![echo command manual](manualecho.png.png)
### ECHO $ COMMAND OUTPUT
![echo $ command output](mecho$home.png)
### ECHO $ COMMAND MANUAL 
![echo $ command manual](manualecho.png.png)
### BC - BASIC CALCULATOR COMMAND OUTPUT
![bc command output](mbc.png)
### BC - BASIC CALCULATOR COMMAND MANUAL
![bc command manual](manualbc.png.png)
## IP FILE COMMANDS
### MAN COMMONAD OUTPUT
![example using man command](manualecho.png.png)
### HISTORY COMMAND OUTPUT
![history command output](mhistory.png)
### HISTORY COMMAND MANUAL 
![history command manual](manualhistory.png.png)
### EXIT COMMAND OUTPUT
![exit command output](mexit.png.png)
### EXIT COMMAND MANUAL
![exit command manual](manualexit.png.png)
## DIRECTIORY COMMANDS
### PWD COMMAND OUTPUT
![pwd command output](mpwd.png)
### PWD COMMAND MANUAL
![pwd command manual](manualpwd.png.png)
### MKDIR COMMAND OUTPUT
![mkdir command output](mmkdirlab.png)
### MKDIR COMMAND MANUAL
![mkdir command manual](manualmkdir.png.png)
### CD  SUBDIR COMMAND OUTPUT
![cd command output](mcdsubdir.png)
### CD SUBDIR COMMAND MANUAL
![cd command manual](manualcdsubdir.png.png)
### CD .. COMMAND OUTPUT
![cd .. command output](mcd...png)
### CD .. COMMAND MANUAL
![cd .. command manual](manualcd.png.png)
### RMDIR COMMAND OUTPUT
![rmdir command output](mrm1.png)
### RMDIR COMMAND MANUAL
![rmdir command manual](manualrmdir.png.png)
## FILE COMMANDS
### CAT COMMAND OUTPUT
![cat command output](mcatfilename.png)
### CAT COMMAND MANUAL
![cat command manual](manualcat.png.png)
### CAT >> COMMAND OUTPUT
![cat >> command output](mcatfilename2.png)
### CAT >> COMMAND MANUAL 
![cat >> command manual](manualcat.png.png)
### CP COMMAND OUTPUT
![cp command output](mcpsrcdes.png)
### CP COMMAND MANUAL
![cp command manual](manualcp.png.png)
### CP -I COMMAND OUTPUT
![cp -i command output](mcp-isrcdes.png)
### CP -I COMMAND MANUAL
![cp -i command manual](manualcp.png.png)
### CP -R COMMAND OUTPUT
![cp -r command output](mcp-rsrcdes.png)
### CP -R COMMAND MANUAL
![cp -r command manual](manualcp.png.png)
### MV OLD NEW COMMAND OUTPUT
![mv command output](mmvoldnew.png)
### MV OLD NEW COMMAND MANUAL
![mv  command manual](manualmv.png.png)
### MV -V OLD NEW COMMAND OUTPUT
![mv  command output](mmv-voldnew.png)
### MV -V OLD NEW COMMAND MANUAL
![mv  command manual](manualmv.png.png)
### RM FILE COMMAND OUTPUT
![rm  command output](mrmfile.png)
### RM FILE COMMAND MANUAL
![rm  command manual](manualrm.png.png)
### RM * COMMAND OUTPUT
![rm * command output](mrm1.png)
### RM * COMMAND MANUAL
![rm * command manual](manualrm.png.png)
### RM -R * COMMAND OUTPUT
![rm -r * command output](mrm-r2.png.png)
### RM * COMMAND MANUAL
![rm -r * command manual](manualrm.png.png)
### RM -F * COMMAND OUTPUT
![rm -f * command output](mrm-f.png.png)
### RM -F * COMMAND OUTPUT
![rm -f * command manual](manualrm.png.png)
### LS COMMAND OUTPUT
![ls command output](mls.png)
### LS COMMAND MANUAL
![ls command manual](manualls.png.png).
### LS NAME * COMMAND OUTPUT
![ls name command output](mlsfile1.png)
### LS NAME * COMMAND MANUAL
![ls name command manual](manualls.png.png)
### LS - A COMMAND OUTPUT
![ls - a command output](mls-a.png)
### LS - A COMMAND MANUAL
![ls -a command manual](manualls.png.png)
### LS -X DIR NAME COMMAND OUTPUT
![ls -x dir command output](mls-xdir.png)
### LS - X DIR NAME COMMAND MANUAL
![ls -x dir command manual](manualls.png.png)
### LS -R COMMAND OUTPUT
![ls -r command manual](mls-R.png)
### LS -R COMMAND MANUAL
![ls -r command manual](manualls.png.png)
### LS - L COMMAND OUTPUT
![ls -l command output](mls-L.png)
### LS -L COMMAND MANUAL
![ls -l command manual](manualls.png.png)
### CMP COMMAND OUTPUT
![cmp command output](mcmpfile1file2.png.png)
### CMP COMMAND MANUAL
![cmp command manual](manualcmp.png.png)
### CHMOD COMMAND OUTPUT
![chmod command output](mchmodg-rfile.png.png)
### CHMOD COMMAND MANUAL
![chmod command manual](manualchmod.png.png)




# The Study And Execution Of Unix Commands Has Been Completed Successfully.
